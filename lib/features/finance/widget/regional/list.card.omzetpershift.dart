...
class ListCardOmzetpershift extends StatelessWidget {
  final List<RegionalOmzetpershiftModel> model;
  final int year;
  final int month;

  const ListCardOmzetpershift({Key key, this.model, this.year, this.month})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    void _barWidget(
        BuildContext context, RegionalOmzetpershiftModel _data) async {
      List<DataRow> listDataTable = _data.data.map(
        (e) {
          var dataRow = DataRow(
            cells: <DataCell>[
              DataCell(Text('${e.id}')),
              DataCell(Text('${e.totalOmset}')),
            ],
          );
          return dataRow;
        },
        ...
        ChartItemModel(
          ...
          chartItem: _data.data
              .map((e) => ChartItem(
                  ...
                  value: double.parse(e.totalOmset)))
              .toList(),
        )
      ];
