...
class ListCardMenu extends StatelessWidget {
  final List<RegionalMenuModel> model;
  final int year;
  final int month;

  const ListCardMenu({Key key, this.model, this.year, this.month})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    void _barWidget(BuildContext context, RegionalMenuModel _data) async {
      List<DataRow> listDataTable = _data.outlets.map(
        (e) {
          var dataRow = DataRow(
            cells: <DataCell>[
              DataCell(Text('${e.id}')),
              DataCell(Text('${e.nama}')),
              DataCell(Text('${e.data.total}')),
            ],
          );
          return dataRow;
        },
      ...
      List<ChartItemModel> _dataChartItem = [
        ChartItemModel(
          ...
          color: 'blue',
          label: 'omzet',
          chartItem: _data.outlets
              .map((e) => ChartItem(
                  date: DateTime(year, month, int.parse(e.id)),
                  value: double.parse(e.data.total)))
              .toList(),
        )
      ];
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return Container(
            ...
            child: ListTile(
              title: Text(_newTitle(dataSelected.menu)),
              subtitle: Text(_newTitle('${dataSelected.kategori}')),
