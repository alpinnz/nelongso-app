...
class ListCardPersentasekunjungan extends StatelessWidget {
  final List<RegionalPersentaseKunjunganModel> model;
  final int year;
  final int month;

  const ListCardPersentasekunjungan(
      {Key key, this.model, this.year, this.month})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    void _barWidget(
        BuildContext context, RegionalPersentaseKunjunganModel _data) async {
      List<DataRow> listDataTable = _data.data.map(
        (e) {
          var dataRow = DataRow(
            cells: <DataCell>[
              DataCell(Text('${e.id}')),
              DataCell(Text('${e.totalKunjungan}')),
            ],
          );
          return dataRow;
        },
        ...
        ChartItemModel(
          ...
          chartItem: _data.data
              .map((e) => ChartItem(
                  ...
                  value: double.parse(e.totalKunjungan)))
              .toList(),
        )
      ];
            ...
            child: Column(
              children: [
                Padding(
                  ...
                  child: Text(
                    'Rekap Persentase Kunjungan Bulanan',
                    style: TextStyle(
