...
class ListCardOperasional extends StatelessWidget {
  final List<RegionalOperasionalModel> model;
  final int year;
  final int month;

  const ListCardOperasional({Key key, this.model, this.year, this.month})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    void _barWidget(
        BuildContext context, RegionalOperasionalModel _data) async {
      // List<DataRow> listDataTable = _data.data.map(
      //   (e) {
      //     var dataRow = DataRow(
      //       cells: <DataCell>[
      //         DataCell(Text('${e.id}')),
      //         DataCell(Text('${e.supplier[0].nominal}')),
      //       ],
      //     );
      //     return dataRow;
      //   },
      ...
      //             value: double.parse(e.supplier[0].nominal)))
      //         .toList(),
      //   )
      // ];
